...
import static reactor.netty5.http.Http2SettingsSpec.SETTINGS_ENABLE_CONNECT_PROTOCOL;
import static reactor.netty5.http.Http2SettingsSpec.TRUE;
...
public final class HttpServerConfig extends ServerTransportConfig<HttpServerConfig> {
	...
	static void addStreamHandlers(Channel ch,
		...
		pipeline.addLast(NettyPipeline.H2ToHttp11Codec, HTTP2_STREAM_FRAME_TO_HTTP_OBJECT)
		        .addLast(NettyPipeline.HttpTrafficHandler,
		                 new Http2StreamBridgeServerHandler(compressPredicate, compressionOptions, formDecoderProvider,
			...
			if (metricsRecorder instanceof HttpServerMetricsRecorder) {
				...
				if (existingHandler != null) {
					...
					if (metricsRecorder instanceof MicrometerHttpServerMetricsRecorder micrometerHttpServerMetricsRecorder) {
						handler = new MicrometerHttpServerMetricsHandler(micrometerHttpServerMetricsRecorder, methodTagValue, uriTagValue);
					}
					else if (metricsRecorder instanceof ContextAwareHttpServerMetricsRecorder contextAwareHttpServerMetricsRecorder) {
						handler = new ContextAwareHttpServerMetricsHandler(contextAwareHttpServerMetricsRecorder, methodTagValue, uriTagValue);
					}
					else {
	...
	static void configureHttp11OrH2CleartextPipeline(ChannelPipeline p,
		...
		p.addBefore(NettyPipeline.ReactiveBridge,
		 ...
		 .addBefore(NettyPipeline.ReactiveBridge,
		            ...
		            new HttpTrafficHandler(compressPredicate, compressionOptions, formDecoderProvider,
	...
	static final class ProtocolHeaderHandler implements ChannelHandler {
