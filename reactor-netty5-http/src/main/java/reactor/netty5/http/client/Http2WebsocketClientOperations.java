...
import io.netty5.handler.codec.http.websocketx.extensions.WebSocketClientExtensionHandshaker;
...
final class Http2WebsocketClientOperations extends WebsocketClientOperations {
	...
	static final class WebsocketClientExtensionHandler implements ChannelHandler {
		...
		public Future<Void> write(ChannelHandlerContext ctx, Object msg) {
			if (!(msg instanceof Buffer)) {
				if (msg.getClass() == DefaultHttpRequest.class) {
					return onHttpRequestWrite(ctx, (DefaultHttpRequest) msg);
				}
				else if (msg instanceof HttpRequest) {
					return onHttpRequestWrite(ctx, (HttpRequest) msg);
				}
				else {
		...
		Future<Void> onHttpRequestWrite(ChannelHandlerContext ctx, HttpRequest request) {
	...
	static final class WebsocketClientHandshaker {
			...
			if (expectedProtocol.isEmpty() && receivedProtocol == null) {
				...
				for (String protocol : expectedProtocol.split(",")) {
					if (protocol.trim().equals(receivedProtocol)) {
						protocolValid = true;
						this.actualSubProtocol = receivedProtocol.toString();
						break;
					}
				}
		...
		Future<Void> handshake(Channel channel) {
			ChannelPipeline pipeline = channel.pipeline();
			ChannelHandlerContext codec = pipeline.context(NettyPipeline.H2ToHttp11Codec);
			if (codec == null) {
				return channel.newFailedFuture(new WebSocketClientHandshakeException(
		...
		static final class WebsocketStreamBridgeClientHandler implements ChannelHandler {
			...
			@Override
			public void channelRead(ChannelHandlerContext ctx, Object msg) {
				if (!(msg instanceof FullHttpResponse) && msg instanceof HttpContent<?> content) {
