...
public abstract class HttpClient extends ClientTransport<HttpClient, HttpClientConfig> {
	...
	public final HttpClient cookiesWhen(Mono<? extends HttpCookiePair> cookieMono) {
		...
		dup.configuration().deferredConf(config ->
			cookieMono == Mono.<HttpCookiePair>empty() ?
					Mono.just(config) :
					cookieMono.map(c -> {
						if (!c.value().isEmpty()) {
							HttpHeaders headers = configuration().headers.copy();
	 ...
	 * {@link reactor.netty5.channel.AbortedException#isConnectionReset(Throwable)}.
	 ...
	 * under the name {@link reactor.netty5.Metrics#HTTP_CLIENT_PREFIX}.
	 ...
	 *     the environment property {@code reactor.netty5.tcp.sslHandshakeTimeout} is set</li>
	...
	public final HttpClient proxyWhen(
		...
		HttpClient dup = duplicate();
		dup.configuration().deferredConf(config -> {
			Mono<? extends ProxyProvider.Builder<?>> mono = proxyBuilder.apply(config, new HttpClientProxyProvider.Build());
