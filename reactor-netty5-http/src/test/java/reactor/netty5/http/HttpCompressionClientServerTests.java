...
import reactor.netty5.http.server.compression.GzipOption;
import reactor.netty5.http.server.compression.ZstdOption;
...
class HttpCompressionClientServerTests extends BaseHttpTest {
	...
	void zstdServerCompressionEnabled(HttpServer server, HttpClient client) {
		...
		assertThat(resp).isNotNull();
		assertThat(getHeader(resp.getT2(), "content-encoding")).isEqualTo("zstd");
					          ...
					          .handle((in, out) ->
					              createClient(port2)
					                        .doOnChannelInit((obs, ch, addr) ->
					                            ch.pipeline().addAfter(HttpCodec, "doTestIssue825_2", new ChannelHandlerAdapter() {
					                                @Override
					                                public void channelRead(ChannelHandlerContext ctx, Object msg) {
					                                    ctx.fireChannelRead(msg);
					                                    if (msg instanceof LastHttpContent<?> last) {
					                                        if (!last.isAccessible()) {
	...
	void serverCompressionWithCompressionLevelSettings(HttpServer server, HttpClient client) {
		...
		try {
			Buffer byteBuf = preferredAllocator().allocate(32);
			byteBuf.writeBytes("reply".getBytes(Charset.defaultCharset()));

			embeddedChannel.writeOutbound(byteBuf);

			try (Buffer encodedByteBuf = embeddedChannel.readOutbound()) {
				byte[] result = new byte[encodedByteBuf.readableBytes()];
				encodedByteBuf.readBytes(result, 0, encodedByteBuf.readableBytes());
