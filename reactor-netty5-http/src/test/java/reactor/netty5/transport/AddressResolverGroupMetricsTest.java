...
class AddressResolverGroupMetricsTest extends BaseHttpTest {
	...
	private double getTimerValue(String address) {
		MeterRegistryAssert.assertThat(registry).hasTimerWithNameAndTags("reactor.netty5.http.client.address.resolver",
				Tags.of(REMOTE_ADDRESS, address, STATUS, SUCCESS));

		Timer timer = registry.find("reactor.netty5.http.client.address.resolver")
