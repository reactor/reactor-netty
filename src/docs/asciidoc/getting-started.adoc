...
It has a transitive dependency on:
...
`Reactor Netty` is part of the `Project Reactor BOM` since `Aluminium` release train.
...
As mentioned earlier, the easiest way to use `Reactor Netty` in your core is to use the `BOM` and
