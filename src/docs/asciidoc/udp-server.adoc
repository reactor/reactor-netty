...
* `flushOnBoundary()` (default): The flush operation will be explicitly invoked on a terminated `Publisher`.
...
The flush operation is not scheduled, which means flush operation is invoked after every write operation.
====
