...
----
include::{sourcedir}/reactor/netty/tcp/TcpClient.java[lines=677..679]
----
====
...
By default, the `TCP` client uses a "`fixed`" connection pool with `500` as the maximum number of channels (for the rest of the configurations check the system properties below).
...
./../../main/java/reactor/netty/ReactorNetty.java
----
include::{sourcedir}/reactor/netty/ReactorNetty.java[lines=111..153]
----
====
