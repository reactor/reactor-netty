...
| <HTTP METHOD>_<URI> | Information and total time for the request/response.
...
See the https://micrometer.io/docs/tracing#_micrometer_tracing_opentelemetry_setup[`Micrometer Tracing`] documentation for `OpenTelemetry` setup if needed.
...
----
<1> Initialises Brave, Zipkin and the Observation registry.
<2> Templated URIs will be used as an URI tag value when possible
<3> Enables the built-in integration with Micrometer
====
