...
By default, Reactor Netty uses a direct memory as this is more performant
...
In order to be able to debug memory issues with the direct memory and the pooled buffers, Netty provides a special memory leak detection mechanism.
Follow the instructions https://github.com/netty/netty/wiki/Reference-counted-objects#troubleshooting-buffer-leaks[Troubleshooting Buffer Leaks]
in order to enable this mechanism. In addition to the instructions provided by Netty, Reactor Netty provides a special
...
By default, this logger is disabled, in order to enable it - increase the log level to `DEBUG`.
