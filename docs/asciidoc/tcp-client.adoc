...
----
include::{examplesdir}/wiretap/custom/Application.java[lines=18..36]
----
<1> Enables the wire logging,
...
NOTE: Netty's HTTP proxy support always uses `CONNECT` method in order to establish a tunnel to the specified proxy regardless of the scheme that is used `http` or `https`.
