...
The below shows the available timeout configuration options:
...
However, you can also configure the connection pool, via `evictInBackground`, to perform periodic checks on connections.
...
NOTE: Reactor Netty uses Reactor Core as Reactive Streams implementation, and you may want to use the `timeout` operator that `Mono`/`Flux` provide.
Keep in mind however that it is better to use more specific timeout configuration options available in Reactor Netty since they provide more control for a specific purpose and use case.
