...
<3> The connection needs to remain idle 5 minutes before TCP starts sending `keepalive` probes.
...
This section describes various timeout configurations supported by `HttpClient`.
Configuring a proper timeout may improve or solve issues in the communication process.
The configurations are grouped as:

* <<connection-pool-timeout-configurations>>
* <<http-client-timeout-configurations>>
** <<response-timeout-configurations>>
** <<connection-timeout-configurations>>
** <<ssl-tls-timeout-configurations>>
** <<proxy-timeout-configurations>>
** <<dns-timeout-configurations>>
...
the connection is returned to the connection pool and thus it can be reused for processing another request. The connection may
be reused immediately for another request or may stay idle in the connection pool for some time.

The following listing shows the available timeout configurations:
...
Choose a configuration equal or less than the one on the target server thus you will be able to reduce the I/O
...
a `PoolAcquireTimeoutException` will be thrown. Default: 45s.

By default, these timeouts are checked on connection `release`/`acquire` operation and if some timeout is reached, the connection is closed and removed from the connection pool.
This default behaviour can be changed with a connection pool's configuration `evictInBackground` thus the connection pool is regularly checked for connections,
...
----
include::{examplesdir}/pool/Application.java[lines=18..50]
----
...
<4> Every 2 minutes the connection pool is regularly checked for connections, that are applicable for removal.
====

[[http-client-timeout-configurations]]
=== HttpClient Timeout Configurations
This section provides information for the various timeout configurations on `HttpClient` level.

NOTE: Reactor Netty uses Reactor Core as Reactive Streams implementation, as such you may want to use the `timeout` operator provided by `Mono`/`Flux`.
Instead of `timeout` operator you should consider using a concrete timeout configuration for the different use cases because when you apply
`timeout` operator it handles the whole operation from establishing the connection to the remote peer to receiving the response.

[[response-timeout-configurations]]
==== Response Timeout Configuration
`HttpClient` provides an API for configuring a default response timeout for all requests. You may change this default response timeout
via an API for a specific request. By default,
...
----
<1> Configures the default response timeout to 1 seconds.
<2> Configures a response timeout for a specific request to 2 seconds.
====

[[connection-timeout-configurations]]
==== Connection Timeout Configurations
The following listing shows the available connection timeout configurations (some of them may apply only to a specific transport)
...
the connection establishment attempt will be failed. Default: 30s.
* `SO_KEEPALIVE` - when the connection stays idle for some time (the time is implementation dependent, but typically 2 hours),
...
if `SO_KEEPALIVE` has been set. The maximum time is implementation dependent, but typically the default is 2 hours.
...
From Reactor Netty point of view in this use case the remote peer just does not respond.
In order to be able to handle such use case you may consider configuring
...
* `handshakeTimeout` - this option can be used for configuring SSL handshake timeout (resolution: ms). Default: 10s.

NOTE: You should consider increasing the SSL handshake timeout when expecting slow network connections.

* `closeNotifyFlushTimeout` - this option can be used for configuring SSL `close_notify` flush timeout (resolution: ms). Default: 3s.
* `closeNotifyReadTimeout` - this option can be used for configuring SSL `close_notify` read timeout (resolution: ms). Default: 0s.
...
[[proxy-timeout-configurations]]
==== Proxy Timeout Configurations
`HttpClient` supports the proxy functionality provided by Netty and provides a way to specify
...
the connection establishment attempt will be failed. Default: 10s.
...
[[dns-timeout-configurations]]
==== Host Name Resolution Timeout Configurations
By default, the `HttpClient` uses Netty’s domain name lookup mechanism that resolves a domain name asynchronously.
...
the DNS server and uses this maximum time to live. Default to `Integer.MAX_VALUE`.
