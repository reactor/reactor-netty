...
| reactor.netty.connection.provider.connections.lifetime | Timer | Time that the connection existed in the connection pool, from its allocation until its destruction.
See xref:observability.adoc#observability-metrics-connections-lifetime[Connections Lifetime]
|=======
