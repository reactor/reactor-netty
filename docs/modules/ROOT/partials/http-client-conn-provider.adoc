...
../../../reactor-netty5-core/src/main/java/reactor/netty5/ReactorNetty.java
[%unbreakable]
----
include::{core}/src/main/java/reactor/netty5/ReactorNetty.java[lines=122..170]
----

When you need to change the default settings, you can configure the `ConnectionProvider` as follows:

[source,java,indent=0]
.{examples-dir}/pool/config/Application.java
[%unbreakable]
----
...
`reactor.netty5.http.client.PrematureCloseException` exception with a root cause "Connect Timeout" due
...
- If you use custom `ConnectionProvider`, invoke {javadoc}/reactor/netty5/resources/ConnectionProvider.html[`ConnectionProvider`]`#dispose`/`#disposeLater`/`#disposeWhen` method.
...
It exposes all metrics with a prefix of `reactor.netty5.connection.provider`.
...
[source,java,indent=0]
.{examples-dir}/pool/metrics/Application.java
[%unbreakable]
----
