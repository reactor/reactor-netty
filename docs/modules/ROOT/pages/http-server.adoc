...
[%unbreakable]
----
include::{sourcedir}/reactor/netty5/http/server/HttpRequestDecoderSpec.java[lines=41..46]
----
...
You can use `-Dreactor.netty5.http.server.errorLogEnabled=true` to enable the `HTTP` error log by configuration.
...
<logger name="reactor.netty5.http.server.ErrorLog" level="ERROR" additivity="false">
    <appender-ref ref="async"/>
</logger>
----
...
For this purpose you need to invoke {javadoc}/reactor/netty5/Metrics.html#observationRegistry-io.micrometer.observation.ObservationRegistry-[`reactor.netty5.Metrics#observationRegistry`].
...
{javadoc}/reactor/netty5/http/server/HttpServer.html#idleTimeout-java.time.Duration-[`idleTimeout`] is not specified,
