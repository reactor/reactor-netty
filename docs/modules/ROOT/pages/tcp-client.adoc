...
:examples-dir: {examples}/src/main/java/reactor/netty5/examples/documentation/tcp/client
:javadoc: https://projectreactor.io/docs/netty/{project-version}/api
:nettyjavadoc: https://netty.io/5.0/api
:wirelogger: reactor.netty5.tcp.TcpClient

[[tcp-client]]
= TCP Client

Reactor Netty provides the easy-to-use and easy-to-configure
{javadoc}/reactor/netty5/tcp/TcpClient.html[`TcpClient`].
...
{javadoc}/reactor/netty5/tcp/TcpClient.html[`TcpClient`] instance.
...
----
<1> Creates a {javadoc}/reactor/netty5/tcp/TcpClient.html[`TcpClient`]
...
The returned {javadoc}/reactor/netty5/Connection.html[`Connection`]
offers a simple connection API, including {javadoc}/reactor/netty5/DisposableChannel.html#disposeNow-java.time.Duration-[`disposeNow()`],
...
The I/O handler has access to {javadoc}/reactor/netty5/NettyOutbound.html[`NettyOutbound`]
...
{javadoc}/reactor/netty5/Connection.html#outbound--[`Connection#outbound`]. As opposed to I/O handler
where the connection is closed when the provided `Publisher` finishes (in case of finite `Publisher`),
when using {javadoc}/reactor/netty5/Connection.html#outbound--[`Connection#outbound`], you have to invoke explicitly
`Connection#dispose` in order to close the connection.

[source,java,indent=0]
.{examples-dir}/send/connection/Application.java
[%unbreakable]
----
...
The I/O handler has access to {javadoc}/reactor/netty5/NettyInbound.html[`NettyInbound`]
...
{javadoc}/reactor/netty5/Connection.html#inbound--[`Connection#inbound`]. As opposed to I/O handler
where the connection is closed when the provided `Publisher` finishes (in case of finite `Publisher`),
when using {javadoc}/reactor/netty5/Connection.html#inbound--[`Connection#inbound`], you have to invoke explicitly
`Connection#dispose` in order to close the connection.

[source,java,indent=0]
.{examples-dir}/read/connection/Application.java
[%unbreakable]
----
...
[source,java,indent=0]
.{sourcedir}/reactor/netty5/tcp/TcpClientConnect.java
----
include::{sourcedir}/reactor/netty5/tcp/TcpClientConnect.java[lines=35..40]
----
...
* {nettyjavadoc}/io.netty5.transport/io/netty5/channel/ChannelOption.html[Common ChannelOption]
* {nettyjavadoc}/io.netty5.transport.classes.epoll/io/netty5/channel/epoll/EpollChannelOption.html[Epoll ChannelOption]
* {nettyjavadoc}/io.netty5.transport.classes.kqueue/io/netty5/channel/kqueue/KQueueChannelOption.html[KQueue ChannelOption]
* https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/net/StandardSocketOptions.html[Standard Socket Options]
* https://docs.oracle.com/en/java/javase/17/docs/api/jdk.net/jdk/net/ExtendedSocketOptions.html[Extended Socket Options]
...
It exposes all metrics with a prefix of `reactor.netty5.tcp.client`.
...
| reactor.netty5.tcp.client.data.received | DistributionSummary | Amount of the data received, in bytes.
See xref:observability.adoc#observability-metrics-data-received[]
| reactor.netty5.tcp.client.data.sent | DistributionSummary | Amount of the data sent, in bytes.
See xref:observability.adoc#observability-metrics-data-sent[]
| reactor.netty5.tcp.client.errors | Counter | Number of errors that occurred.
See xref:observability.adoc#observability-metrics-errors-count[]
| reactor.netty5.tcp.client.tls.handshake.time | Timer | Time spent for TLS handshake.
See xref:observability.adoc#observability-metrics-tls-handshake-time[]
| reactor.netty5.tcp.client.connect.time | Timer | Time spent for connecting to the remote address.
See xref:observability.adoc#observability-metrics-connect-time[]
| reactor.netty5.tcp.client.address.resolver | Timer | Time spent for resolving the address.
